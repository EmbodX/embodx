...
use dimensify::{
    robot_vis::{DesireRobotState, EndEffectorMode, EndEffectorTarget, RobotState},
    WebAssetPlugin,
};
...
fn main() -> Result<()> {
    ...
    // #[cfg(target_arch = "wasm32")]
    {
        ...
        fn update_target(
            ...
            mut q_robot_state_entity: Query<(Entity), With<RobotState>>,
            ...
            if (time.elapsed() - previous_time.elapsed()) < Duration::from_millis(5) {
                return;
            }
            *previous_time = *time;
            ...
            if let Some((mut transform, mut ee_target)) = end_eff {
                if device_state.control_position {
                    ee_target.translation = match ee_target.translation_mode {
                        ...
                        EndEffectorMode::ApplyAsDelta => {
                            // let relative_translation = device_state.translation - reference_origin.0;
                            // Some(
                            //     device_state.rotation * relative_translation
                            //     )
                            ...
                            // // let relative_translation = device_state.translation - reference_origin.0;
                            // // // rotate to get the relative translation in the reference frame
                            // // Some(device_state.rotation * relative_translation);
                            ...
                            // error!("> {:#?}", relative_translation);


                            // Some(relative_translation)

                            // // if relative_translation.length() < std::f32::EPSILON {
                            // //     None
                            // // } else {
                            // //     error!("> {:#?}", relative_translation);
                            //     error!("+ {:#?}", device_state.rotation * relative_translation);
                ...
                ;
                // FIXME: modify the component location of endeffectortarget
                // commands.entity(q_robot_state_entity.single())
                // .insert(DesireRobotState::from())
